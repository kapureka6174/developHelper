<template>
    <div>
        <div class="m-3 pb-3 bg-blue-400 rounded">
            <div class="flex items-center">
                ...
                <!-- 削除ボタン -->
                <svg
                    class="mr-3 w-6 h-6 hover:text-red-400 text-red-600"
            ...
            <div class="mb-3 w-full text-center">
                <button
                    class="w-40 h-10 text-white hover:bg-indigo-400 bg-indigo-600 rounded"
            ...
            <require-input
                v-for="(requirement, requirementIndex) in pages[index].requirements"
        ...
        <server-error
            ...
            :errors="
                Object.entries($page.props.errors).filter((e) => {
                    return e[0].split('.')[0] == 'pages' && e[0].split('.')[1] == `${index}`
                        ? true
                        : false;
                })
            "
    ...
    export default {
        ...
        setup(props) {
            ...
            const input = (value) => {
                // 空入力かどうかのチェック
                if (!value) {
                    pages[props.index].pagename.error = "ページ名が入力されていません。";
                } else if (
            ...
            const addSelectedRequirement = (url, pageIndex) => {
                ...
                if (url.split("/")[1] == "create") {
                    ...
                    selectedArray.forEach((requirement) => {
                        pages[pageIndex].requirements.push(requirement.title.content);
                    });
                } else {
                    selectedArray.forEach((requirement) => {
                        if (
